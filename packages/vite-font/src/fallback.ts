 ...
 * Weights can be defined as a number, 'normal' or 'bold'.
...
function getDistanceFromNormalWeight(weight?: string) {
  if (!weight) return 0
  // Variable fonts define weight with two numbers "100 900", rather than just one "400"
  ...
  // If the weight doesn't have a second value, it's not a variable font
  ...
  if (Math.abs(firstWeightDistance) < Math.abs(secondWeightDistance)) return firstWeightDistance
  return secondWeightDistance
}

/**
 * If multiple font files are provided for a font family, we need to pick one to use
...
export function pickFontFileForFallbackGeneration<T extends { style?: string; weight?: string; metadata: Font }>(fontFiles: T[]): T {
  return fontFiles.reduce((usedFontFile, currentFontFile) => {
    ...
    if (absUsedDistance === absCurrentDistance && currentFontDistance < usedFontDistance) return currentFontFile
    return usedFontFile
  })
}
